...
export class OrderByPipe<T extends Record<string, string>> implements PipeTransform { 
  ...
  transform(value: any): OrderByDto | undefined {
    ...
    const allowedValues = Object.values(this.allowedEnum).map((v) => v.toLowerCase());
    const allowedDirections = Object.values(OrderByDirection).map((v) => v.toLowerCase());

    // Validate field and direction values.
    if (!allowedValues.includes(field.toLowerCase())) {
      throw new BadRequestException(`Invalid value: ${field}. Allowed values: ${allowedValues.join(', ')}`);
    }
    
    if (!allowedDirections.includes(direction.toLowerCase())) {
      throw new BadRequestException(`Invalid order direction: ${direction}. Allowed values: ${allowedDirections.join(', ')}`);
    }

    // Return field value from Enum.
    return {field: this.allowedEnum[field.toUpperCase()], direction: direction};
  }
}
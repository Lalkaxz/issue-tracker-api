...
import { getRepositoryName, isGitHubRepoUrl } from 'src/common/utils/parse-url.util';
...
export class ProjectsService {
  ...
  async create(projectDto: CreateProjectDto, user: UserEntity): Promise<Project> {
    ...
    const projectName = projectDto.name || getRepositoryName(projectDto.repoUrl);
  ...
  async delete(project: ProjectEntity,
               user: UserEntity
  ): Promise<Project> {
    if (project.ownerId !== user.id) {
